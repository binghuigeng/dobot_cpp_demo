...
{
    unsigned short crc_check = 0xFFFF;
    unsigned int uIndex ;
    while (len --)
    {
        uIndex = (crc_check & 0xff) ^ ((*pBuf) & 0xff);
        pBuf = pBuf + 1;
        crc_check = ((crc_check >> 8) & 0xff) ^ crc16_tab[uIndex];
    }
    return crc_check ;
}
 ...
 * @file util.cpp
 * @brief 法三：CRC校验码计算，
 * @note 经测试，此校验方法非瑞尔特力控使用的CRC校验算法
 */
...
unsigned short CRC16(const unsigned char* pBuf, unsigned short len)
{
    ...
    while (len--) {
        crc_check = crc16_table[((crc_check >> 8) ^ *pBuf++) & 0xff] ^ (crc_check << 8);
    }
    return crc_check;
}
#endif
